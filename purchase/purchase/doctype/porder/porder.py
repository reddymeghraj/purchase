...
class Porder(Document):
	...
	pass
@frappe.whitelist()
def get_item_info(item,brand,sqt):
	query=frappe.db.sql("""select item_name,quantity,rate,discount,taxes,price,amount from `tabAdd Sqt` where parent=%s and brand=%s and item=%s""",(sqt,brand,item),as_dict=1)
	return query
